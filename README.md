...
Ainz Toolkit provides a local prompt library, character profiles, reusable tag sets, Booru imports, local image previews, style references, smart collections and direct prompt insertion into NovelAI.
...
Network requests are limited to supported websites and are used only when required for the currently opened post, image import or an explicitly started refresh or repair action.
